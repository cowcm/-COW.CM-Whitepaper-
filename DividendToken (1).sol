...
contract DividendToken is ERC20, Ownable, BaseToken {
    ...
    mapping(address => bool) public isMaxBuyExempt;
    ...
    event maxBuyAmountSet(uint256 _amount);
    ...
    constructor(
        ...
        uint8 decimal_, // 18
        address dividendTrackerAddress_, // 0x9795DcF2CfEe83456bCD4801eeB7a7f9c7A7551B
        ...
        maxBuyAmount = totalSupply_.div(1000); //1 M
        ...
        isMaxBuyExempt[msg.sender] = true;
        isMaxBuyExempt[address(uniswapV2Router)] = true;
        isMaxBuyExempt[address(uniswapV2Pair)] = true;
        isMaxBuyExempt[address(this)] = true;
        isMaxBuyExempt[address(DEAD)] = true;
        isMaxBuyExempt[address(ZERO)] = true;
    ...
    function _transfer(
        ...
        if (automatedMarketMakerPairs[from] && !isMaxBuyExempt[to]) {
            require(
                balanceOf(to) + amount <= maxBuyAmount,
                "Can not buy more tokens than maxBuyAmount"
            );
        }
    ...
    function setIsMaxBuyExempt(
        ...
        require(
            _address != address(uniswapV2Router) &&
                ...
                _address != address(DEAD) &&
                _address != address(ZERO),
            "Cannot modify pre-set exempt addresses"
        );
        isMaxBuyExempt[_address] = exempt;
    }
    ...
    function setMaxBuyAmount(uint256 _amount) external onlyOwner {
        require(
            _amount >= totalSupply().div(1000),
            "Can not set buy limit less than 1 million tokens"
        );
        maxBuyAmount = _amount;

        emit maxBuyAmountSet(_amount);
    }
